...
import { Home, Users, ListTodo, RefreshCw, AlertCircle, ExternalLink } from 'lucide-react';
...
export default function App() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setErrorMsg("Akses Ditolak. Silakan klik tombol 'Login ke Google' di bawah.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="flex min-h-screen bg-[#1c1c1e] text-gray-300 font-sans tracking-tight">
      <aside className="w-[240px] bg-[#222225] border-r border-white/5 p-6 hidden md:block">
        <h1 className="text-white font-bold text-xl mb-10 text-blue-500">HR AGENTIC</h1>
      ...
      <main className="flex-1 p-6 md:p-10">
        <div className="max-w-5xl mx-auto space-y-6">
          <header className="flex justify-between items-center mb-8">
            <div>
              <h1 className="text-2xl font-bold text-white">Daily Schedule 2026</h1>
              <p className="text-gray-400 text-sm italic">Status: {isLoading ? "Syncing..." : "Live Connection"}</p>
            </div>
          ...
          {errorMsg && (
            <div className="bg-amber-500/10 border border-amber-500/20 p-6 rounded-xl space-y-3 shadow-xl">
              <div className="flex items-center gap-3 text-amber-400 font-bold text-sm">
                <AlertCircle size={20} /> Keamanan Google Workspace Terdeteksi
              </div>
              <p className="text-xs text-gray-400 leading-relaxed">
                Karena kebijakan kantor, browser perlu memastikan Anda adalah pemilik data. Klik tombol di bawah untuk membuka akses, lalu kembali ke sini dan Refresh.
              </p>
              <a 
                href={webAppUrl} 
                target="_blank" 
                rel="noreferrer"
                className="inline-flex items-center gap-2 bg-amber-600 text-white px-4 py-2 rounded-lg text-xs font-bold hover:bg-amber-700 transition-all shadow-lg shadow-amber-900/20"
              >
                <ExternalLink size={14} /> Buka Izin Akses (Login Google)
              </a>
            </div>
          )}
          ...
          <div className="bg-[#242427] rounded-xl border border-white/5 overflow-hidden shadow-2xl mt-8">
            <div className="p-4 border-b border-white/5 bg-white/5 flex items-center justify-between">
              <h3 className="text-xs font-bold text-white uppercase tracking-widest flex items-center gap-2">
                <ListTodo size={16} className="text-blue-400"/> Task Timeline
              </h3>
              <span className="text-[9px] bg-blue-500/20 text-blue-400 px-2 py-1 rounded border border-blue-500/20 italic">TAB: ALL 2026</span>
            </div>
            
            <div className="divide-y divide-white/5">
              {!isLoading && taskData.length === 0 ? (
                <div className="p-20 text-center text-gray-500 italic">Data belum muncul. Klik tombol sinkronisasi di atas.</div>
              ) : (
                taskData.map((task) => (
                  <div key={task.id} className="p-4 flex flex-col md:flex-row md:items-center justify-between hover:bg-white/[0.02] gap-4 transition-colors">
                    <div className="flex gap-4 items-start flex-1">
                      <div className={`mt-1.5 w-2.5 h-2.5 rounded-full flex-shrink-0 ${task.ceklis === 'Done' ? 'bg-emerald-500' : 'bg-red-500 shadow-[0_0_8px_rgba(239,68,68,0.4)]'}`} />
                      <div className="space-y-1">
                        <p className={`text-sm font-semibold tracking-tight ${task.ceklis === 'Done' ? 'text-gray-500 line-through' : 'text-gray-100'}`}>
                          {task.taskList}
                        </p>
                        <p className="text-[11px] text-gray-500 italic max-w-lg">{task.detailTindakan}</p>
                        <div className="flex gap-2 pt-1">
                          <span className="text-[9px] px-2 py-0.5 bg-blue-500/10 text-blue-400 rounded border border-blue-500/20 uppercase">{task.kategori}</span>
                          {task.urgent === 'Y' && <span className="text-[9px] px-2 py-0.5 bg-red-500/20 text-red-400 rounded font-bold tracking-tighter">URGENT</span>}
                        </div>
                      </div>
                    </div>
                    <button 
